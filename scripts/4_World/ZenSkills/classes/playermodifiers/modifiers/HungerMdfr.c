modded class HungerMdfr
{
	override void OnTick(PlayerBase player, float deltaT)
	{
		...
		PlayerStat<float> hungerStat = player.GetStatEnergy();
		float before = hungerStat.Get();
		...
		float newVal = Math.Clamp(after + refund, 0, PlayerConstants.SL_ENERGY_MAX);
		hungerStat.Set(newVal);

		#ifdef ZENSKILLSDEBUG
		ZenSkillsPrint("[HungerPerk] pct=" + pct + " before=" + before + " after=" + after + " decay=" + decay + " refund=" + refund + " new=" + newVal);
		#endif
	}
}
